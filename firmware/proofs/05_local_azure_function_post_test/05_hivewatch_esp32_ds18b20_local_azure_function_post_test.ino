...
// Local Azure Function endpoint placeholder
const char* endpointUrl = "http://YOUR-LOCAL-FUNCTION-HOST:7215/api/IngestTelemetry";
...
void postTemperatureToLocalFunction(float temperatureC) {
  WiFiClient client;
  HTTPClient http;
  ...
  Serial.println("Sending HTTP POST to local HiveWatch Azure Function...");
  ...
  if (http.begin(client, endpointUrl)) {
    ...
    Serial.println("Unable to initialise HTTP connection.");
  }
}
...
void setup() {
  ...
  postTemperatureToLocalFunction(temperatureC);
